...
namespace SpriteShatter {
    ...
    public class Shatter : MonoBehaviour {
        ...
        void Start() {
            ...
            try {
                sourcePixels = spriteRenderer.sprite.texture.GetPixels((int) spriteRenderer.sprite.rect.xMin, (int) spriteRenderer.sprite.rect.yMin, spriteWidth,
                        spriteHeight);
            }
            catch {
                Debug.LogError("Sprite Shatter (" + name + "): There was an error reading the texture data. Please ensure the \"Read/Write\" texture flag is " +
                        "set.");
                error = true;
                return;
            }
                    ...
                    for (int k = 0; k < textureWidth; k++)
                        for (int l = 0; l < textureHeight; l++)
                            if ((j == 0 || k >= verticalCutLeft[j - 1, l]) && (j == shatterDetails.verticalCuts || k < verticalCutLeft[j, l]) &&
                                ...
                                if (k > maxVerticalCutLeft)
                                    maxVerticalCutLeft = k;
                                if (l < minHorizontalCutTop)
                                    minHorizontalCutTop = l;
                                if (l > maxHorizontalCutTop)
                                    maxHorizontalCutTop = l;
                            }
                    ...
                    if (maxVerticalCutLeft - minVerticalCutLeft > 0 && maxHorizontalCutTop - minHorizontalCutTop > 0) {
                        ...
                        if (!allTransparent) {
                            ...
                            spriteShatterGameObject.transform.localPosition = new Vector3(spriteRenderer.bounds.center.x -
                                    spriteRenderer.bounds.extents.x + ((float) (minVerticalCutLeft * spriteWidth) / (float) textureWidth /
                                    spriteRenderer.sprite.pixelsPerUnit), spriteRenderer.bounds.center.y - spriteRenderer.bounds.extents.y +
                                    ((float) (minHorizontalCutTop * spriteHeight) / (float) textureHeight / spriteRenderer.sprite.pixelsPerUnit),
                            ...
                            shatterSpriteRenderer.sprite = Sprite.Create(shatterTexture, new Rect(0, 0, shatterTexture.width, shatterTexture.height),
                                    Vector2.zero, ((float) textureWidth / (float) spriteWidth) * spriteRenderer.sprite.pixelsPerUnit);
                            ...
                            if (shatterDetails.colliderType == ColliderType.Polygon) {
                                ...
                                boxCollider.offset = new Vector2(
                                        (float) (textureMinX + textureMaxX + 1) / (spriteRenderer.sprite.pixelsPerUnit * 2),
                                        (float) (textureMinY + textureMaxY + 1) / (spriteRenderer.sprite.pixelsPerUnit * 2));
                                boxCollider.size = new Vector2((float) (textureMaxX - textureMinX) / spriteRenderer.sprite.pixelsPerUnit,
                                        (float) (textureMaxY - textureMinY) / spriteRenderer.sprite.pixelsPerUnit);
                                ...
                                circleCollider.offset = new Vector2(
                                        (float) (textureMinX + textureMaxX + 1) / (spriteRenderer.sprite.pixelsPerUnit * 2),
                                        (float) (textureMinY + textureMaxY + 1) / (spriteRenderer.sprite.pixelsPerUnit * 2));
                                circleCollider.radius = ((textureMaxX - textureMinX) + (textureMaxY - textureMinY)) /
                            ...
                            if (colliderCouldNotBeCreated)
                                GameObject.Destroy(spriteShatterGameObject);
                            else {
        ...
        public void makeCuts(Sprite sprite, int horizontalCuts, int verticalCuts, int randomSeed, float randomness, float zigzagFrequency,
            ...
            int zigzagFrequencyPixels = zigzagFrequency < 0.0001 ? 0 : ((int) (((float) ((float) Math.Max(textureWidth, textureHeight)) *
                    (1 - (zigzagFrequency * 0.975f))) / 4) * 4);
            int zigzagAmplitudePixels = (int) (Mathf.Min((float) textureHeight / (float) (verticalCuts + 1), (float) textureWidth /
                    (float) (horizontalCuts + 1)) * zigzagAmplitude);
                    ...
                    for (int j = 0; j < (k == 0 ? textureWidth : textureHeight); j++) {
                        ...
                        if (k == 0)
                            horizontalCutTop[i, j] = Math.Min(Math.Max((((i + 1) * textureHeight) / (horizontalCuts + 1)) +
                                    ...
                                    randomTopOffsets[i, 0] + (int) (zigzag * (float) zigzagAmplitudePixels), 0), textureHeight - 1);
                        else
                            verticalCutLeft[i, j] = Math.Min(Math.Max((((i + 1) * textureWidth) / (verticalCuts + 1)) +
                                    ...
                                    randomLeftOffsets[i, 0] + (int) (zigzag * (float) zigzagAmplitudePixels), 0), textureWidth - 1);
                    }
                }
        }
        ...
        static float transition(int minValue, int maxValue, int currentValue, bool useTrigonometryAtStart, bool useTrigonometryAtEnd, float scale) {
            ...
            if (useTrigonometryAtStart && useTrigonometryAtEnd)
                return (float) ((Mathf.Sin((((float) (currentValue - minValue) / (float) (maxValue - minValue)) * Mathf.PI) - (Mathf.PI / 2)) / 2) + 0.5) *
                        scale;
            else if (!useTrigonometryAtStart && !useTrigonometryAtEnd)
                return ((float) (currentValue - minValue) / (float) (maxValue - minValue)) * scale;
            else {
                float multiplier = Mathf.Clamp((((float) (currentValue - minValue) / (float) (maxValue - minValue)) * 2) - 0.5f, 0, 1) * scale;
                float trigonometryAmount = (float) ((Mathf.Sin((((float) (currentValue - minValue) / (float) (maxValue - minValue)) * Mathf.PI) -
                        (Mathf.PI / 2)) / 2) + 0.5);
                float nonTrigonometryAmount = (float) (currentValue - minValue) / (float) (maxValue - minValue);
