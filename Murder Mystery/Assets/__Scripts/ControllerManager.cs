...
public class ControllerManager : MonoBehaviour {
	...
	void Update () {
		for(int i = 0; i < allControllers.Count; i++){
			...
			if(allControllers[i].Action1){//X which is detective 1
			...
			if(allControllers[i].Action2){//O which is detective 2
			...
			if(allControllers[i].Action3){//Square which is ghost 1
			...
			if(allControllers[i].Action4){//Triangle which is ghost 2
